...
function customize_login_page_language() {
    $locale = 'cs_CZ';
    // Use absolute path to ensure correct file location.
    $translation_path = get_template_directory() . '/languages' . $locale . '.pot';

    // Debug: Check if the path is correct.
    error_log("Translation file path: $translation_path");

    // Load the translation file for Czech.
    // Make sure to check for errors while loading the translation.
    $loaded = load_textdomain( 'login-customizer', $translation_path );

    // Debug: Check if the translation is loaded successfully.
    if ( ! $loaded ) {
        $error = sprintf("Failed to load translation file for Czech. Error: %s", print_r($loaded, true));
        error_log($error);
    }
}
// Hook the function to the 'login_init' action.
add_action( 'login_init', 'customize_login_page_language' );
